...
3. When prompted, fill in the `sync: false` env vars:
   ...
   - **Order emails:** set `RESEND_API_KEY` from [Resend](https://resend.com). Set `RESEND_FROM` to a verified sender (e.g. `CloseAndKeep <orders@yourdomain.com>`). Each new gift order emails `ORDER_NOTIFICATION_TO` (defaults to `CloseAndKeep@gmail.com` if unset).
